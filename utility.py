...
def compute_utility_stats(flash_df, he_df, smoke_df, molotov_df,
    ...
    # Assign rounds to all grenade events
    ...
    # Flash assists from kills_df (assistedflash == True)
    ...
    # Enemies blinded: kills where attackerblind == True means the VICTIM was flashed
    # We use assister_name from assistedflash kills to count who flashed effectively
    # Better: count unique (round, assister) pairs where assistedflash=True
    effective_flashes = (
    ...
    # Enemies blinded per flash thrown (effectiveness rate)
    # Use flash_assists / flashes_thrown
    ...
    # ── Combined per-player utility table ─────────────────────────────────────
